...
import { updateTracksList } from '../ws-server';
...
export const updateTrackById: RouteHandler<UpdateTrackParams> = async (
  ...
  try {
    ...
    const { title, artist, album, genres, coverImage } = request.body;
    request.log.info({ id, title, artist, album, genres, coverImage }, 'Updating track');
    ...
    request.log.info({ updatedTrack }, 'Track updated successfully');
